        ...
        <tr className={styles.tableRow}>
            <td>
                <Link href={`users/${username}`}>
                    {organisationName}</Link> </td>
            <td ><Link href={`users/${username}`}>{username}</Link></td>
            <td >{email}</td>
            <td>{phoneNumber}</td>
            <td >{dateJoined}</td>
            <td className={`${styles.pill} ${styles[status]}`}>{status}</td>
            <td>
                <button className={styles.options} role="menu" aria-label="View, Activate or block a user" onClick={() => setIsVisible(!isVisible)}>
